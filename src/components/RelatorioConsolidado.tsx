...
export default function RelatorioConsolidado({ secretariaId, onBack }: ConsolidatedTableProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const deptSnap = await getDocs(query(collection(db, 'departamentos'), where('secretariaId', '==', secretariaId), orderBy('nome', 'asc')));
      const depts = deptSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Departamento));
      setDepartamentos(depts);

      const guiasSnap = await getDocs(query(
        collection(db, 'guias'), 
        where('mes', '==', mes),
        where('ano', '==', ano)
      ));
        ...
        <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row md:justify-between md:items-center gap-8">
          <div>
            ...
            <p className="flex items-center gap-2 mt-2 text-gray-400 font-black text-[8px] uppercase tracking-[0.2em]">
               <RotateCcw className="w-3 h-3 text-gray-200" /> Relatório Consolidado de Previdência
            </p>
          </div>
